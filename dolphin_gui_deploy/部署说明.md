...
unzip -o tag_system_dolphin.zip -d tag_system/
echo "✅ 部署包解压完成到: /dolphinscheduler/default/resources/tag_system/"
ls -la tag_system/main.py tag_system/src/ tag_system/requirements.txt
...
```bash
#!/bin/bash
cd /dolphinscheduler/default/resources/tag_system
...
**任务名称**: `health_check`
**任务类型**: Spark
**依赖任务**: `install_dependencies`（或者`unzip_deploy_package`如果跳过依赖安装）
...
echo "✅ 部署包解压完成"
ls -la main.py src/ requirements.txt
```
