...
class TagTaskFactory:
    ...
    @classmethod
    def create_task(cls, tag_id: int, task_config: Dict[str, Any], spark: SparkSession, system_config) -> BaseTagTask:
        """
        根据标签ID创建对应的任务实例
        ...
        if spark is None or system_config is None:
            raise ValueError("创建任务需要提供spark和system_config参数")
        
        task_class = cls._task_registry[tag_id]
        return task_class(task_config, spark, system_config)
