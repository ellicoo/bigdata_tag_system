...
def generate_comprehensive_test_data(spark) -> bool:
    ...
    try:
        ...
        for i in range(user_count):
            ...
            register_time = (datetime(2020, 1, 1) + timedelta(days=random.randint(0, 1500))).strftime(
                "%Y-%m-%d %H:%M:%S")
        ...
        for i in range(user_count):
            ...
            total_position = str(
                int(spot_position) + int(contract_position) + int(finance_position) + int(onchain_position))
        ...
        asset_df = spark.createDataFrame(asset_data, [
            ...
            "spot_position_value", "contract_position_value", "finance_position_value", "onchain_position_value",
            ...
            "spot_locked_amount", "contract_locked_amount", "finance_locked_amount", "onchain_locked_amount",
            "locked_amount"
        ])
        ...
        trading_df = spark.createDataFrame(trading_data, [
            "user_id", "spot_trading_volume", "contract_trading_volume", "spot_recent_30d_volume",
            "contract_recent_30d_volume",
            "spot_trade_count", "contract_trade_count", "finance_trade_count", "onchain_trade_count",
            "first_trade_time", "last_trade_time", "has_contract_trading", "contract_trading_style",
            "has_finance_management", "has_pending_orders"
        ])
